...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Username desde sessionStorage:', username); // Agregar log
    ...
    if (username && welcomeMessage) {
        console.log('Mostrando nombre en welcomeMessage');
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                data.forEach(item => {
                    ...
                    const readIcon = document.createElement('ion-icon');
                    readIcon.name = 'book';
...
function cargarDatosUsuarios(apiEndpoint, tableBodyId, columnCount) {
    const username = sessionStorage.getItem('username'); // Obtenemos el usuario activo
        ...
        .then(data => {
            ...
            if (!data || data.length === 0) {
                ...
                data.forEach(item => {
                    ...
                    birthDateCell.textContent = item.nacimiento ? formatDate(item.nacimiento) : 'Fecha no disponible';
                    ...
                    editButton.addEventListener('click', function () {
                        abrirModalEdicionUsuario(item.id);
                    });
                    ...
                    // Deshabilitar botón de eliminar para el usuario activo
                    if (username === item.usuario) {
                        deleteButton.disabled = true; // Deshabilitar si es el usuario activo
                        deleteButton.style.opacity = 0.5; // Darle un estilo para que se vea deshabilitado
                    }

                    deleteButton.addEventListener('click', function () {
                        const userId = deleteButton.dataset.id;
                        eliminarUsuario(userId, '/api/users', row);
                    });
                ...
                // Si ya existe una instancia de DataTable, se actualiza
...
function eliminarUsuario(id, apiEndpoint, row) {
    if (confirm('¿Estás seguro de que deseas eliminar este usuario? ')) {
        ...
        .then(response => {
            if (!response.ok) {
                // Si el estado de la respuesta no es 2xx, lanzamos un error
        ...
        .then(data => {
            ...
            // Eliminar la fila de la tabla si se elimina con éxito
            if (row) {
                row.remove();
            }

            // Opcional: Recargar los datos de la tabla después de eliminar
            cargarDatosUsuarios(apiEndpoint, 'users-tbody', 8); // Ajusta según sea necesario
        })
        .catch(error => {
